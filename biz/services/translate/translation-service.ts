import { CHECK_PARAM_ERROR_CODE, DB_ERROR_CODE, SUCCESS_CODE, UNAUTHORIZED_ERROR_CODE } from "@/types/dto/response";
...
import { SubmitTaskData, TranslationHistoryPage, TranslationTaskDetailView } from "@/types/dto/translation-task";
import { TranslationImageView, TranslationImagePollView } from "@/types/dto/translation-image";
...
import { estimateCreditPerImage } from "@/biz/utils/credits";
import { createTranslationBackend } from "./backends/create-translation-backend";
import { TranslationImageInput } from "./backends/translation-backend";
...
export class TranslationService {
    ...
    // 提交任务
    async submitTranslationTask(taskId: string, images: File[], intent: TranslationIntent): Promise<BizResult<SubmitTaskData>> {
        // 参数检查
        ...
        // 2. 获取价格配置
        const creditPerImage = estimateCreditPerImage(intent);
    ...
    async batchTranslatePipeline(task: TranslationTask, images: TranslationImage[]) {
        ...
        // 1. 下载原始图片
        const { validImages, blobs } = await this.downloadOriginalImages(images);
        ...
        // 2. 提交到Modal服务，先固定使用 MIT
        const inputs: TranslationImageInput[] = [];
        const today = new Date().toISOString().slice(0, 10);
        for (let i = 0; i < validImages.length; i++) {
            const validImage = validImages[i];
            const blob = blobs[i];
            const finalFilePath = `${task.userId}/${today}/output/${task.id}_${validImage.imageIndex}.webp`;
            inputs.push({
        ...
        const streamEvents = submitResult.data!;
